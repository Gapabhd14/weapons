...
let alertTextWeapon = [`1) טרם הבדיקה הטכנית נדרש לבצע בדיקת פריקה לכלי הנשק עפ"י הוראות מבק"א <br> 2) אסורה הימצאות תחמושת למעט תחמושת תרגול (כדורי תרגול) באזור התיקון והטיפול בכלי הנשק. <br>
3) מחסן כלי הנשק חייב להיות מאוורר בעת העבודה. <br> 4) ניקוי כלי הנשק יבוצע בתערובת 9345 בלבד. <br> 5) במחסן הנשק או במקום תיקון הנשק ניתן לפרוק כלי נשק אישיים ואקדחים לתוך ארגז חול ים.
...
const openAlert = (event) => {
    ...
    document.getElementById("textAlert").innerHTML = `<p>${alertTitleWeapon} <br> ${alertTextWeapon}</p>`;
